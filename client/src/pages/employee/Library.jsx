...
export default function Library() {
  ...
  const loadSchedule = async () => {
    setLoading(true);
    try {
      const r = await api.get('/requests/library-schedule', { params: { from: dates[0], to: dates[4] } });
      setSchedule(r.data);
    } catch (e) {}
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <h2 className="text-xl font-bold">לוח ספריה</h2>
        ...
        {loading ? <p className="text-gray-400 text-sm">טוען...</p> : (
          <div className="space-y-2">
            {dates.map(date => {
              ...
              return (
                <div key={date} className={`border rounded-xl p-3 ${isToday ? 'border-blue-300 bg-blue-50' : ''}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-semibold text-sm">{dayName} {date}</span>
                    {isToday && <span className="badge badge-blue text-xs">היום</span>}
                  </div>
                  {bookings.length === 0 ? (
                    <span className="text-green-600 text-sm">ספריה פנויה</span>
                  ) : (
      ...
      <div className="card">
        ...
        <div className="mb-3">
          ...
          <input className="input w-full" value={form.notes}
            onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="..." />
        </div>
        ...
        {conflictInfo && (
          <div className="bg-orange-50 border border-orange-300 rounded-lg px-3 py-3 mb-3 space-y-2">
            ...
            <input className="input w-full text-sm" placeholder="סיבה לצורך בספרייה (למשל: פגישה פרטית, עבודה שקטה...)"
            ...
            <div className="flex gap-2">
              <button className="btn btn-primary text-sm" disabled={!conflictReason.trim() || submitting}
                onClick={() => book(conflictReason, conflictInfo.original_room_id)}>גרע מהחדר הרגיל ורשום לספריה</button>
