...
router.post('/', requirePerm('rooms'), (req, res) => {
...
router.put('/:id', requirePerm('rooms'), (req, res) => {
...
router.delete('/:id', requirePerm('rooms'), (req, res) => {
  db.get('rooms').find({ id: +req.params.id }).assign({ is_active: false }).write();
  res.json({ message: 'החדר הושבת' });
});

router.post('/upload', requirePerm('rooms'), upload.single('file'), (req, res) => {
...
router.get('/files', requirePerm('rooms'), (req, res) => {
